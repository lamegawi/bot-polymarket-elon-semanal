...
ESTADO = "papel_semanal.json"
HISTORIAL = "resultados_papel_semanal.csv"
BANKROLL = 500.0
...
def resolver(estado):
    ...
    try:
        notificar.apuesta_cerrada(registro, estado["saldo"], prefijo="[SEMANAL] ")
    except Exception:
        pass
    return True
...
def abrir(estado, actualizar=False):
    ...
    candidatas = [c for c in candidatas if c["tipo"] == "semanal"]
    if not candidatas:
        print(f"  (sin señal semanal → no se abre apuesta de papel · paso actual: {estado['paso']})")
        return False
    c = candidatas[0]
    ...
    try:
        notificar.apuesta_abierta(estado["activa"], estado["saldo"], prefijo="[SEMANAL] ")
    except Exception:
        pass
    return True
...
def pasada(actualizar=False, excel=False):
    ...
    print(f"[{datetime.now(ET).strftime('%Y-%m-%d %H:%M %Z')}] Paper trading SEMANAL · "
